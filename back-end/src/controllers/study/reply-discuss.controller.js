...
const replyDiscussService = require("../../services/study/reply-discuss.service");
...
exports.deleteReplyDiscussByAuthorCourse = async (req, res, next) => {
  try {
    const token = req.cookies.token;
    var userInfor = await jwt.verify(token, process.env.JWT_SECRET_KEY);
    const result = await replyDiscussService.deleteReplyDiscussByAuthorCourse(
      req.params.reply_id,
      req.params.discuss_id,
      userInfor._id
    );
